...
box1 = Object(m, x.copy(), v, debug=False)
box2 = Object(m, x + 50 * Y, v, debug=False)
box3 = Object(m, x + 50 * X, v, debug=False)
box4 = Object(m, x + 50 * Y + 50 * X, v, debug=False)

initial = [x.copy(), x + 50 * Y, x + 50 * X, x + 50 * Y + 50 * X]
points = [box1, box2, box3, box4]

def block_stasis(obj):
    f = ZERO.copy()
    for i, point in enumerate(points):
        if point is not obj:
            f -= 1000*((initial[i] - initial[points.index(obj)]) - (point.x - obj.x))
    return f
...
# SIMULATE
with World(*points, size=SIZE, slowdown=0, recording=True, recording_interval=1/FPS) as world:
    world.run(8)
